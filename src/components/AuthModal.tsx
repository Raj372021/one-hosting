...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const { loginWithDetails, loginWithGoogle, resetUserPassword } = useAuth();
  ...
  const handleGoogleLogin = (googleEmail: string, googleName: string, role: 'user' | 'admin' = 'user') => {
    loginWithGoogle(googleEmail, googleName, undefined, role);
    showToast(`✅ Authenticated with Google Account (${googleEmail})`, 'success');
  ...
  const handleCustomGoogleSubmit = (e: React.FormEvent) => {
    ...
    const isAdmin = customGoogleEmail.toLowerCase().includes('admin');
    const computedName = customGoogleEmail.split('@')[0].replace(/[^a-zA-Z0-9]/g, ' ').trim() || 'Google User';
    handleGoogleLogin(customGoogleEmail.trim(), computedName, isAdmin ? 'admin' : 'user');
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          ...
          <div className="flex items-center gap-2">
            {mode !== 'signin' && (
              <button
                ...
                className="px-2.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-bold flex items-center gap-1 transition-all cursor-pointer"
      ...
      {isGoogleSelecting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-150">
          <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4 text-slate-100 shadow-2xl">
            <div className="text-center space-y-1.5">
              ...
              <div className="text-xs text-slate-400">Select an account or enter your Google email</div>
            </div>
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
              ...
              <button
                type="button"
                onClick={() => handleGoogleLogin('rajsahani.RgcS@gmail.com', 'Raj Sahani', 'user')}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-xs text-white group-hover:text-indigo-400 transition-colors flex items-center gap-1.5">
                    <span>Raj Sahani</span>
                    <span className="text-[9px] bg-indigo-500/20 text-indigo-300 px-1.5 py-0.5 rounded font-bold">User</span>
                  </div>
              ...
              <button
                type="button"
                onClick={() => handleGoogleLogin('rajsahani.RgcS@gmail.com', 'Super Admin (Raj Sahani)', 'admin')}
                className="w-full p-3 rounded-2xl bg-purple-950/30 hover:bg-purple-900/40 border border-purple-500/30 text-left flex items-center gap-3 transition-colors group cursor-pointer"
              >
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center font-bold text-white text-xs">
                  SA
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-xs text-purple-300 group-hover:text-white transition-colors flex items-center gap-1.5">
                    <span>Super Admin Access</span>
                    <span className="text-[9px] bg-purple-500/30 text-purple-200 px-1.5 py-0.5 rounded font-black">ADMIN</span>
                  </div>
                  <div className="text-[11px] text-slate-400 truncate">rajsahani.RgcS@gmail.com</div>
                </div>
              </button>
              ...
              {!isCustomGoogleInputOpen ? (
                <button
                  ...
                  className="w-full p-2.5 rounded-xl bg-slate-950/60 hover:bg-slate-800 border border-dashed border-slate-700 text-xs font-semibold text-slate-300 hover:text-white flex items-center justify-center gap-2 cursor-pointer transition-colors"
                >
                  <UserIcon className="w-3.5 h-3.5 text-slate-400" />
                  <span>Use Another Google Account</span>
                </button>
              ) : (
                <form onSubmit={handleCustomGoogleSubmit} className="p-3 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
                  <div className="text-[11px] font-bold text-slate-300">Enter Google Account Email:</div>
                  <input
                    type="email"
                  ...
                  <div className="flex gap-2">
                    <button
                      type="submit"
                      className="flex-1 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs cursor-pointer"
                    >
                      Continue
                    </button>
                    <button
                      type="button"
                      onClick={() => setIsCustomGoogleInputOpen(false)}
